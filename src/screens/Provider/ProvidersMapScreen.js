...
import { ArrowRight, MagnifyingGlass, Funnel, Crosshair, Star, Wrench, WarningCircle, SmileySad, ArrowsDownUp } from 'phosphor-react-native';
...
export default function ProvidersMapScreen({ navigation }) {
  ...
  const categories = useMemo(() => {
    const set = new Set();
    providers.forEach((p) => (p.serviceCategories || []).forEach((c) => set.add(c)));
  ...
  return (
    <View style={s.root}>
      <LinearGradient colors={['#f2ecf8', '#e7ddf3']} start={{ x: 0.2, y: 0 }} end={{ x: 0.8, y: 1 }} style={StyleSheet.absoluteFill}>
        ...
        {shown.slice(0, PIN_POS.length).map((p, i) => {
          const on = p.id === selected?.id;
          return (
            <Pressable key={p.id} onPress={() => setSelectedId(p.id)} style={[s.provPin, PIN_POS[i], on && s.provPinOn]}>
      ...
      <View style={s.topBar}>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <View style={s.search}>
            ...
            <TextInput value={q} onChangeText={setQ} placeholder="ابحث عن فني" placeholderTextColor={colors.textMuted2} textAlign="right" style={s.searchInput} />
          </View>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, flexDirection: 'row-reverse' }}>
          {/* المسافة */}
          <Pressable onPress={() => setDistIdx((i) => (i + 1) % DISTANCES.length)}>
          ...
          {/* التقييم (فرز محلي) */}
          <Pressable onPress={() => setRatingSort((v) => !v)} style={ratingSort ? null : s.filterOff}>
            {ratingSort ? (
              <LinearGradient colors={gradients.primary} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.filterOn}>
          ...
          {/* الأنواع */}
          {category && (
            <Pressable onPress={() => setCategory(null)} style={s.filterOff}><Text style={s.filterOffText}>× {category}</Text></Pressable>
          )}
          {categories.filter((c) => c !== category).map((c) => (
            <Pressable key={c} onPress={() => setCategory(c)} style={s.filterOff}><Text style={s.filterOffText}>{c}</Text></Pressable>
          ))}
        </ScrollView>
      </View>

      <Pressable style={s.fab} onPress={() => load(category, DISTANCES[distIdx])}>
      ...
      {!loading && error && (
        <View style={s.stateCard}>
          ...
          <Pressable style={s.retry} onPress={() => load(category, DISTANCES[distIdx])}><Text style={s.retryText}>إعادة المحاولة</Text></Pressable>
        </View>
      )}
      {!loading && !error && shown.length === 0 && (
        <View style={s.stateCard}><SmileySad size={34} weight="fill" color={colors.textMuted2} /><Text style={s.stateText}>لا يوجد فنيون ضمن هذا النطاق</Text></View>
      )}

      {/* بطاقة الفني المحدّد */}
      {!loading && !error && selected && (
        <View style={s.provCard}>
          <Pressable onPress={() => navigation?.navigate?.('ProviderProfile', { providerId: selected.id })} style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 13, marginBottom: 14 }}>
          ...
          <Pressable onPress={() => navigation?.navigate?.('Services', { providerId: selected.id })} style={({ pressed }) => pressed && { transform: [{ scale: 0.97 }] }}>
...
const s = StyleSheet.create({
  ...
  searchInput: { flex: 1, minWidth: 0, fontSize: 13.5, color: '#2a2333', padding: 0 },
  filterOn: { flexDirection: 'row-reverse', alignItems: 'center', gap: 5, borderRadius: 999, paddingVertical: 8, paddingHorizontal: 13, ...shadow.soft, shadowOffset: { width: 0, height: 4 } },
  filterOnText: { fontSize: 12, fontWeight: '700', color: '#fff' },
  filterOff: { backgroundColor: '#fff', borderRadius: 999, paddingVertical: 8, paddingHorizontal: 13, justifyContent: 'center', ...shadow.soft, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.14 },
